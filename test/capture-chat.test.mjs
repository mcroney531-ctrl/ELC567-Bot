...
/* Each slice runs in its own frame against one origin, which is how the stages
   will sit on the timeline: separate conversations, one shared store. */
const ROLES = ['problem', 'coach-workflow', 'coach-tools', 'draft'];
const site = await serveSite(8147);
...
let turns = { 'coach-workflow': 0, 'coach-tools': 0 };
...
const stored = () => page.evaluate(() => JSON.parse(
  document.querySelector('#coach-workflow').contentWindow.localStorage
...
const restart = async () => {
  ...
  await F('problem').locator('#bw-reset').click();
  await settle(1600);
  turns = { 'coach-workflow': 0, 'coach-tools': 0 };
};
...
try {
  await page.goto('http://127.0.0.1:8147/frames/' + ROLES.join(','));
  await settle(1200);
  ...
  check('its opening quotes the task they named',
    (await lastBot('coach-workflow')).includes('Every Monday'));

  // ------------------------------------------------- prose, said out loud
  let r = await say('coach-workflow',
  ...
  // ------------------------------------------------------- correcting one
  r = await say('coach-workflow', 'step 2 should be draft each update from the CRM notes');
  ...
  // --------------------------------------------------------- adding later
  r = await say('coach-workflow', 'oh and then I email it to the account leads');
  ...
  // -------------------------------------------------------------- confirm
  r = await say('coach-workflow', 'yes that looks right');
  ...
  // ----------------------------------------------------- tools, downstream
  await settle(1400);
  await bots('coach-tools', 1); turns['coach-tools'] = 1;
  check('the tools chat unlocked itself',
    await F('coach-tools').locator('#bw-chat-wrap').isVisible());
  check('and it opens by reading back the list block 3 wrote',
    (await lastBot('coach-tools')).includes('Email it to the account leads'));

  r = await say('coach-tools', 'Tableau, Word, PowerPoint, Outlook');
  ...
  await nameTask();
  await bots('coach-workflow', 1); turns['coach-workflow'] = 1;
  await say('coach-workflow', '1. Pull the numbers\n2. Draft the update\n3. Reformat the deck');
  ...
  r = await say('coach-tools', 'Excel and Word');
  ...
  r = await say('coach-tools', 'it is all in Excel actually');
  ...
  await nameTask();
  await bots('coach-workflow', 1); turns['coach-workflow'] = 1;
  r = await say('coach-workflow', 'I rebuild the deck');
  check('a single step is pushed on rather than accepted',
    /one step isn'?t a workflow/i.test(r), r.slice(0, 200));
  r = await say('coach-workflow', 'before that I export the numbers');
  ...
  // ------------------------------------------------------ restart is local
  await F('coach-workflow').locator('#bw-chat-restart').click();
  await F('coach-workflow').locator('#bw-chat-restart').click();
  await settle(1200);
  data = await stored();
